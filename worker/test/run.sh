...
#
#   worker/test/run.sh e2e     hits the live GitHub API, writes /tmp/*.svg
...
case "$MODE" in
  e2e)
    ...
    node --experimental-strip-types "$WORK/src/e2e.ts" /tmp/card.svg /tmp/languages.svg
    echo "  wrote /tmp/card.svg and /tmp/languages.svg — open them to check the render"
    ;;
  bench)
